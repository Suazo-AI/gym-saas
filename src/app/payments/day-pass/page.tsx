...
export default async function DayPassPage({ searchParams }: Props) {
  ...
  if (!params.gymMemberId) {
    ...
    return <AppShell activeGym={activeGym} currentPath="/payments" userEmail={user.email}>
      ...
      <section className="mt-6 rounded-lg border border-charcoal bg-paper p-5 shadow-sm">
        <form className="flex flex-col gap-3 sm:flex-row"><input className="min-h-11 flex-1 rounded-md border border-gray px-3 text-ink" defaultValue={params.search ?? ""} name="search" placeholder="Buscar por nombre o código" /><button className="min-h-11 rounded-md bg-ink px-5 py-3 text-sm font-black text-paper" type="submit">Buscar</button></form>
  ...
  if (!result) return <AppShell activeGym={activeGym} currentPath="/payments" userEmail={user.email}><p className="mt-6 rounded-lg bg-red-50 p-5 font-bold text-red-800">No pudimos cargar el miembro.</p></AppShell>;
  const [member, methods, passes] = result;
  if (!member) return <AppShell activeGym={activeGym} currentPath="/payments" userEmail={user.email}><p className="mt-6 rounded-lg border border-charcoal bg-paper p-5 font-bold text-charcoal">No encontramos el miembro en este gimnasio.</p></AppShell>;
  return <AppShell activeGym={activeGym} currentPath="/payments" userEmail={user.email}>
