...
export function LocalizedNav({ screens, currentPath }: { screens: Array<{ code: string; name: string; route: string }>; currentPath?: string }) {
  ...
  return <nav className="mt-8 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-1">{screens.map((screen) => <Link aria-current={activePath === screen.route ? "page" : undefined} className={`rounded-md px-4 py-3 text-sm font-bold transition ${activePath === screen.route ? "bg-brand-green text-white shadow-sm" : "text-gray-light hover:bg-white/10 hover:text-white"}`} href={screen.route} key={screen.route}>{screenLabel(screen.code, screen.name, locale)}</Link>)}</nav>;
}
...
function screenLabel(code:string,fallback:string,locale:Locale){const labels:Record<string,[string,string]>={dashboard:["Resumen","Dashboard"],members:["Miembros","Members"],memberships:["Membresías","Memberships"],payments:["Pagos","Payments"],income:["Ingresos","Income"],entries:["Entradas","Entries"],facial_access:["Acceso facial","Facial access"],staff:["Personal","Staff"],settings:["Configuración","Settings"]};return labels[code]?.[locale==="es"?0:1]??fallback;}
